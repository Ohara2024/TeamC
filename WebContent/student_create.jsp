    ...
    <div class="content">
        ...
        <% String error = request.getParameter("error");
           ...
           if ("duplicate".equals(error)) {
               errorMessage = "学生番号が重複しています";
           } else if ("invalid".equals(error)) {
               errorMessage = "入力内容が無効です。すべての項目を正しく入力してください。";
        ...
        <form action="StudentServlet" method="POST" onsubmit="return validateForm()">
            ...
            <div class="form-group">
                ...
                <span id="entrance_year_warning" class="warning">入学年度を入力してください</span>
            </div>
                ...
                <select name="class_num" id="class_num" required onchange="updateSchoolCd()">
                    ...
                    <%
                       ...
                       try {
                           ...
                           conn = DriverManager.getConnection("jdbc:h2:tcp://localhost/~/seiseki", "sa", "");
                           stmt = conn.createStatement();
                           String query = "SELECT SCHOOL_CD, CLASS_NUM FROM CLASS_NUM ORDER BY CLASS_NUM";
                           rs = stmt.executeQuery(query);
                           while (rs.next()) {
                               String schoolCd = rs.getString("SCHOOL_CD");
                               String classNum = rs.getString("CLASS_NUM");
                    %>
                               <option value="<%= classNum %>" data-school-cd="<%= schoolCd %>" <%= classNum.equals(selectedClass) ? "selected" : "" %>><%= classNum %></option>
                    <%
                           }
